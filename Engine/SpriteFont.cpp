...
constexpr auto MAX_TEXTURE_RESOLUTION = 4096;
...
namespace Engine
{
    SpriteFont::SpriteFont(const char* font, int size, char cs, char ce)
    {
        ...
        for (char c = cs; c <=ce; c++)
        {
            ...
            glyphRects[i].x = 0;
            glyphRects[i].x -= glyphRects[i].y;
            glyphRects[i].y = 0;
            i++;
        }

        int rows = 1, w, h, bestWidth = 0, bestHeight = 0,  area = MAX_TEXTURE_RESOLUTION * MAX_TEXTURE_RESOLUTION, bestRows = 0;
        ...
        while (rows <= m_regLength)
        {
            ...
            if (w > MAX_TEXTURE_RESOLUTION || h > MAX_TEXTURE_RESOLUTION)
            {
                rows++;
                delete[] gr;
                continue;
        ...
        memset(pureWhiteSquare, 0xffffff, rs * rs * sizeof(int));
        ...
        for (int i=0; i<m_regLength;i++)
        {
            m_glyphs[i].character = (char)(cs + i);
            m_glyphs[i].size = glm::vec2(glyphRects[i].z, glyphRects[i].w);
            m_glyphs[i].uvRect = glm::vec4((float)glyphRects[i].x / (float)bestWidth,
